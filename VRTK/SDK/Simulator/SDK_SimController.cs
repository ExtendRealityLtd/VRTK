...
namespace VRTK
{
    ...
    public class SDK_SimController : SDK_BaseController
    {
        ...
        /// <summary>
        /// checks if the given button (KeyCode) is currently in a touched state
        /// Touched states are achieved by holding down the button defined as touch modifier
        /// while hitting the desired Button.
        ///
        /// E.g. to get only a touch event on the Touchpad (no press), one has to hold down the TouchModifier
        /// while pressing the button defined as TouchpadPress. See keyMappings for details.
        /// </summary>
        /// <param name="index">unique index of the controller for which the button touch is to be checked</param>
        /// <param name="type">the type of touch (up, down, hold)</param>
        /// <param name="button">the button on the keyboard</param>
        /// <returns></returns>
        private bool IsButtonTouched(uint index, ButtonPressTypes type, KeyCode button)
        {
            bool buttonTouch = false;
            switch (type)
            {
                case ButtonPressTypes.Touch:
                    buttonTouch = Input.GetKey(button);
                    break;
                case ButtonPressTypes.TouchDown:
                    buttonTouch = Input.GetKeyDown(button);
                    break;
                case ButtonPressTypes.TouchUp:
                    buttonTouch = Input.GetKeyUp(button);
                    break;
            }
            return buttonTouch && IsButtonPressed(index, ButtonPressTypes.Press, keyMappings["TouchModifier"]);
        }
