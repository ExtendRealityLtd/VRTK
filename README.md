...
The following script parameters are available:
  ...
  * **Teleport Beam:** A custom Game Object can be applied
  here to appear only if the teleport is allowed (its material will not
  be changed ).
  * **Rescale Pointer Tracer:** Rescale each pointer tracer element
  according to the length of the Bezier curve.
